  ...
  return (
    <div className="card" style={{ marginTop: '16px' }}>
      ...
      <p style={{ opacity: 0.8, marginBottom: '16px' }}>
        Detect CD Digital Audio tracks in ISO files (may not work properly in OPL)
      </p>
      ...
      {result && (
        <div className="card" style={{ 
          ...
          <h4 style={{ 
            margin: '0 0 12px 0',
            color: result.has_audio ? 'var(--ui-warning)' : 'var(--neo-accent-3)'
          }}>
            {result.has_audio ? '⚠️ Audio Detected' : '✅ No Audio Detected'}
          </h4>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '14px' }}>
            <div><strong>Audio Tracks:</strong> {result.audio_tracks}</div>
            <div><strong>Audio Data:</strong> {result.total_audio_mb.toFixed(2)} MB</div>
          </div>
