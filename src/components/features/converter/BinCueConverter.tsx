  ...
  const selectCueFile = async () => {
    try {
      ...
      if (selected) {
        ...
        // Get conversion info
        try {
          const conversionInfo = await invoke<ConversionInfo>(
            'get_conversion_info',
            {
              cuePath: selected,
            },
    ...
    <div className="card" style={{ marginTop: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        <Input
          ...
          style={{ flex: 1 }}
        />
        <Button onClick={selectCueFile}>Browse...</Button>
      </div>

      {info && (
        <div
          className="card"
          style={{ background: 'rgba(76, 194, 255, 0.05)', padding: '16px' }}
        >
          ...
          >
            <div>
              <strong>BIN File:</strong> {info.bin_path}
            </div>
            <div>
            ...
            <div>
              ...
              {(info.bin_size / 1048576).toFixed(2)} MB
            </div>
            <div>
              <strong>Output Size:</strong>{' '}
              {(info.output_size / 1048576).toFixed(2)} MB
            </div>
            <div>
              <strong>Needs Conversion:</strong>{' '}
              {info.needs_conversion ? 'Yes' : 'No (already ISO)'}
            </div>
          </div>
      ...
      >
        ...
        <ul style={{ margin: '8px 0 0 0', paddingLeft: '20px' }}>
          ...
          <li>
            If needed, converts RAW sectors (2352 bytes) to ISO (2048 bytes)
          </li>
